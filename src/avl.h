...
// 绘点，仅画父节点，由父节点向下延伸
void draw(ImVec2 pos, ImVec2 p1, ImVec2 p2, std::string msg) {
    ImDrawList *draw_list = ImGui::GetWindowDrawList();
    ImColor color {255, 0, 0, 255}; 
    draw_list->AddLine(pos, p1, color);
    draw_list->AddLine(pos, p2, color);
    // 坐标，圆的半径，颜色，线宽
    draw_list->AddCircle(pos, r, color, 0, 1.0f);
...
void dfs(Node* &p, ImVec2 pos) {
    ...
    dfs(p->l, p1);
    draw(pos, p1, p2, std::to_string(p->key));
    dfs(p->r, p2);
}
